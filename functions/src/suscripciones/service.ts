...
export async function actualizarBorradorPlan(db: Firestore, entrada: Envelope & { planId: string; planVersion: number; capacidades: string[]; limites: PlanVersion["limites"]; periodicidad: PlanVersion["periodicidad"]; grandfathered: boolean }, ctx: ContextoComercial) {
  validar(entrada); if (!esIdComercial(entrada.planId) || !Number.isInteger(entrada.planVersion) || entrada.planVersion < 1 || !Array.isArray(entrada.capacidades) || entrada.capacidades.some(x => !esIdComercial(x)) || !["MENSUAL", "ANUAL", "SIN_VENCIMIENTO"].includes(entrada.periodicidad) || typeof entrada.grandfathered !== "boolean" || !entrada.limites || typeof entrada.limites !== "object") fail("invalid-argument", "PLAN_INVALIDO");
  const f = hash(entrada); return db.runTransaction(async tx => { const p = await previo(tx, db, entrada, "PLATFORM", f); if (p) return { ...p, idempotente: true }; const ref = db.collection("planes").doc(entrada.planId).collection("versiones").doc(String(entrada.planVersion)); const snap = await tx.get(ref); if (!snap.exists) fail("not-found", "PLAN_NOT_FOUND"); const plan = snap.data() as PlanVersion; if (plan.revision !== entrada.expectedRevision || plan.estado !== "BORRADOR") fail("failed-precondition", "PLAN_TRANSITION_INVALID"); const revision = plan.revision + 1; tx.update(ref, { capacidades: entrada.capacidades, limites: entrada.limites, periodicidad: entrada.periodicidad, grandfathered: entrada.grandfathered, revision, actualizadaEn: FieldValue.serverTimestamp() }); const r = { planId: plan.planId, planVersion: plan.planVersion, revision }; registrar(tx, db, entrada, "PLATFORM", f, r, "BorradorPlanActualizado", "PLAN", plan.revision, revision, ctx); return { ...r, idempotente: false }; });
}
...
export async function renovarSuscripcion(db: Firestore, entrada: Envelope & { empresaId: string; periodoInicio: string; periodoFin: string }, ctx: ContextoComercial) {
  ...
  const f = hash(entrada); return db.runTransaction(async tx => { const p = await previo(tx, db, entrada, entrada.empresaId, f); if (p) return { ...p, idempotente: true }; const ref = db.collection("suscripciones").doc(entrada.empresaId), snap = await tx.get(ref); if (!snap.exists) fail("not-found", "SUSCRIPCION_NOT_FOUND"); const s = snap.data() as Suscripcion; if (s.revision !== entrada.expectedRevision || s.estado !== "active") fail("failed-precondition", "SUSCRIPCION_RENEWAL_INVALID"); const revision = s.revision + 1; tx.update(ref, { periodoInicio: entrada.periodoInicio, periodoFin: entrada.periodoFin, revision, actualizadaEn: FieldValue.serverTimestamp() }); const r = { empresaId: s.empresaId, revision }; registrar(tx, db, entrada, s.empresaId, f, r, "SuscripcionRenovada", "SUSCRIPCION", s.revision, revision, ctx); return { ...r, idempotente: false }; });
}
...
export async function programarCancelacionSuscripcion(db: Firestore, entrada: Envelope & { empresaId: string; cancelacionProgramadaPara: string }, ctx: ContextoComercial) {
  ...
  const f = hash(entrada); return db.runTransaction(async tx => { const p = await previo(tx, db, entrada, entrada.empresaId, f); if (p) return { ...p, idempotente: true }; const ref = db.collection("suscripciones").doc(entrada.empresaId), snap = await tx.get(ref); if (!snap.exists) fail("not-found", "SUSCRIPCION_NOT_FOUND"); const s = snap.data() as Suscripcion; if (s.revision !== entrada.expectedRevision || s.estado !== "active" || (s.periodoFin && entrada.cancelacionProgramadaPara > s.periodoFin)) fail("failed-precondition", "CANCELACION_INVALIDA"); const revision = s.revision + 1; tx.update(ref, { cancelacionProgramadaPara: entrada.cancelacionProgramadaPara, revision, actualizadaEn: FieldValue.serverTimestamp() }); const r = { empresaId: s.empresaId, revision }; registrar(tx, db, entrada, s.empresaId, f, r, "SuscripcionCancelacionProgramada", "SUSCRIPCION", s.revision, revision, ctx); return { ...r, idempotente: false }; });
}
