  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-card border-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground">Pendientes</p><p className="text-xl font-bold">{cuentas.filter(c => c.estado === "pendiente").length}</p></CardContent></Card>
        <Card className="bg-card border-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground">Total pendiente</p><p className="text-xl font-bold text-amber-400">{formatCurrency(totalP)}</p></CardContent></Card>
      </div>
      <div className="flex gap-2">
        {(["pendiente", "pagada", "todas"] as const).map(f => <button key={f} onClick={() => setFiltro(f)} className={cn("px-3 py-1.5 rounded-lg text-xs font-medium", filtro === f ? "bg-primary text-primary-foreground" : "bg-secondary/50 text-muted-foreground")}>{f === "pendiente" ? "Pendientes" : f === "pagada" ? "Pagadas" : "Todas"}</button>)}
      </div>
      {filtradas.length === 0 ? <Card className="bg-card border-border"><CardContent className="py-8 text-center text-muted-foreground">Sin cuentas</CardContent></Card> : (
        <div className="space-y-2">{filtradas.map(c => (
          <Card key={c.id} className={cn("bg-card border-border", c.estado === "pendiente" && "border-l-2 border-l-amber-500")}><CardContent className="p-3">
            <div className="flex items-center justify-between mb-1">
              <p className="font-medium text-foreground text-sm">{c.clienteNombre}</p>
              <Badge className={cn(c.estado === "pendiente" ? "bg-amber-500/20 text-amber-400" : "bg-emerald-500/20 text-emerald-400")}>{c.estado === "pendiente" ? "Pendiente" : "Pagada"}</Badge></div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-amber-400 text-sm">{formatCurrency(c.totales?.total || 0)}</span></div>
          </CardContent></Card>
        ))}</div>
      )}
    </div>
