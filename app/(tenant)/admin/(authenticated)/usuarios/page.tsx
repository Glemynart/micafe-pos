...
export default function UsuariosPage() {
  ...
  return (
    <div className="pb-4">
      {/* Header */}
      <div className="px-4 pt-5 pb-4 border-b border-border/50 bg-card/50 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Ajustes</h1>
          <p className="text-xs text-muted-foreground mt-0.5">Configuración y gestión</p>
        </div>
        <Button
          size="sm"
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div>
          ...
          <div className="bg-card/50 border border-border rounded-xl overflow-hidden divide-y divide-border/50">
            {[
              ...
              return (
                <Link key={item.href} href={item.href} className="flex items-center gap-3 px-4 py-3 hover:bg-card/50 transition-colors">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link href="/admin/mermas" className="bg-card/50 border border-border rounded-xl p-4 hover:bg-card/50 transition-colors">
              ...
              <p className="text-sm font-semibold text-foreground/80">Mermas</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">Pérdidas registradas</p>
            </Link>
            <Link href="/admin/gastos" className="bg-card/50 border border-border rounded-xl p-4 hover:bg-card/50 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-orange-500/20 flex items-center justify-center mb-3">
                <TrendingDown className="h-4 w-4 text-orange-400" />
              </div>
              <p className="text-sm font-semibold text-foreground/80">Gastos</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">Salidas y responsables</p>
            </Link>
            <Link href="/admin/cuentas-cobro" className="bg-card/50 border border-border rounded-xl p-4 hover:bg-card/50 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center mb-3">
                <ClipboardList className="h-4 w-4 text-amber-400" />
              </div>
              <p className="text-sm font-semibold text-foreground/80">Cuentas</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">Por cobrar</p>
            </Link>
            <Link href="/admin/compras" className="bg-card/50 border border-border rounded-xl p-4 hover:bg-card/50 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-sky-500/20 flex items-center justify-center mb-3">
                <Truck className="h-4 w-4 text-sky-400" />
              </div>
              <p className="text-sm font-semibold text-foreground/80">Compras</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">A proveedores</p>
            </Link>
            <Link href="/admin/eventos" className="bg-card/50 border border-border rounded-xl p-4 hover:bg-card/50 transition-colors">
              ...
              <p className="text-sm font-semibold text-foreground/80">Eventos</p>
          ...
          <div className="bg-card/50 border border-border rounded-xl overflow-hidden divide-y divide-border/50">
            ...
            {usuarios.map(u => (
              <div key={u.uid} className="px-4 py-3 flex items-center gap-3">
                {/* Avatar */}
                <div className="w-8 h-8 rounded-full bg-card/50 flex items-center justify-center shrink-0">
                  <span className="text-xs font-bold text-muted-foreground">{(u.nombre || u.username).charAt(0).toUpperCase()}</span>
                </div>
                {/* Info */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-foreground/80 truncate">{u.nombre}</p>
        ...
        {/* Security note */}
        <div className="bg-card/50 border border-border rounded-xl px-4 py-3 flex items-start gap-3">
          <Lock className="h-4 w-4 text-muted-foreground shrink-0 mt-0.5" />
          <p className="text-xs text-muted-foreground">Los operadores usan código + PIN para iniciar sesión. Desactivar un operador impide su acceso inmediatamente. El PIN temporal debe cambiarse en el primer ingreso.</p>
        </div>
      </div>

      {/* Dialog crear operador */}
      <Dialog open={showCreate} onOpenChange={(open) => { if (!open) setShowCreate(false) }}>
        <DialogContent className="!bg-background !text-foreground !border-border" showCloseButton={false}>
          <DialogHeader>
            <DialogTitle className="text-foreground font-bold">Nuevo Operador</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div>
              <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Nombre completo</Label>
              <Input
                className="mt-1.5 !bg-input !border-border !text-foreground placeholder:text-muted-foreground focus-visible:ring-primary"
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setShowCreate(false)} className="!border-border !text-foreground/70 hover:!bg-card/50">Cancelar</Button>
            <Button onClick={hCreate} disabled={creando} className="!bg-primary !text-primary-foreground hover:!bg-primary/90 shadow-none font-bold">
      ...
      <Dialog open={showCredential} onOpenChange={() => {}}>
        <DialogContent className="!bg-background !text-foreground !border-border [&>button]:hidden" showCloseButton={false}>
          <DialogHeader>
            <DialogTitle className="text-foreground font-bold">Credencial del operador</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 bg-input border border-border rounded-lg px-3 py-2 text-sm text-foreground font-mono">{credentialData?.codigo}</code>
                <Button
                  size="sm"
                  variant="outline"
            ...
            {credentialData?.pinTemporal && (
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <code className="flex-1 bg-input border border-border rounded-lg px-3 py-2 text-sm text-foreground font-mono tracking-widest">{showPin ? credentialData.pinTemporal : "••••••"}</code>
                  <Button
                    size="sm"
                    variant="outline"
                    className="!border-border !text-foreground/70 hover:!bg-card/50 h-9"
          ...
          <DialogFooter>
            <Button onClick={hCloseCredential} className="!bg-primary !text-primary-foreground hover:!bg-primary/90 shadow-none font-bold">
