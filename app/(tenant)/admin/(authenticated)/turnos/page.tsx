    ...
    <div className="pb-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/20 p-4">
          ...
          <p className="text-2xl font-black text-foreground mt-1">{abiertos.length}</p>
          <p className="text-[11px] text-muted-foreground mt-1">turnos abiertos</p>
        </div>
        <div className="rounded-2xl bg-card/50 border border-border p-4">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Cerrados</p>
          <p className="text-2xl font-black text-foreground mt-1">{cerrados.length}</p>
          <p className="text-[11px] text-muted-foreground mt-1">en el historial visible</p>
        </div>
        <div className="rounded-2xl bg-red-500/10 border border-red-500/20 p-4">
          ...
          <p className="text-[11px] text-muted-foreground mt-1">{turnosConAlerta.filter((turno) => (turno.diferenciaEfectivo || 0) < 0).length} cierres</p>
        </div>
      ...
      <div className="space-y-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-card/50 border border-border rounded-xl pl-9 pr-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50"
          />
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                filtro === value
                  ...
                  : "bg-card/50 border-border text-muted-foreground/80 hover:text-foreground/80",
              )}
            >
              {label}
            </button>
      ...
      {turnos.length === 0 ? (
        <div className="rounded-2xl bg-card/50 border border-border py-12 text-center text-sm text-muted-foreground">
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-2xl bg-card/50 border border-border divide-y divide-border/50 overflow-hidden">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-bold text-foreground truncate">{nombreCajero(turno)}</p>
                      </div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5 mt-3">
                    {[
                      ...
                      <div key={item.label} className={cn("rounded-lg p-2", item.alert ? "bg-red-500/10" : "bg-card/50")}>
                        <p className="text-[9px] text-muted-foreground/80 uppercase tracking-wide font-bold">{item.label}</p>
                        <p className={cn("text-[11px] font-black mt-0.5 tabular-nums", item.alert ? "text-red-300" : "text-foreground/75")}>
                          {item.value}
                        </p>
                      </div>
                    ))}
                  ...
                  <div className="flex items-center justify-between mt-3 text-[11px] text-muted-foreground">
                    ...
                    <span className="inline-flex items-center gap-1 text-primary font-bold">Ver detalle <ChevronRight className="h-3.5 w-3.5" /></span>
                  </div>
                </button>
              )
            })}
      ...
      {seleccionado && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center sm:justify-center" onClick={() => setSeleccionado(null)}>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-bold text-muted-foreground/80 uppercase tracking-widest">Detalle del turno</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-xl bg-card/50 p-3">
                <p className="text-[10px] text-muted-foreground/80 uppercase font-bold">Apertura</p>
                <p className="text-xs font-semibold text-foreground/80 mt-1">{formatDateTime(seleccionado.fechaApertura)}</p>
              </div>
              <div className="rounded-xl bg-card/50 p-3">
                <p className="text-[10px] text-muted-foreground/80 uppercase font-bold">Cierre</p>
                <p className="text-xs font-semibold text-foreground/80 mt-1">{formatDateTime(seleccionado.fechaCierre)}</p>
              </div>
            </div>
            ...
            <section>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Cuadre de caja</h3>
              </div>
              <div className="rounded-xl border border-border overflow-hidden divide-y divide-border/50">
                ...
                <div className={cn("flex items-center justify-between px-3 py-3", seleccionado.estado === "abierto" ? "bg-card/50" : (seleccionado.diferenciaEfectivo || 0) < 0 ? "bg-red-500/10" : (seleccionado.diferenciaEfectivo || 0) > 0 ? "bg-amber-500/10" : "bg-emerald-500/10")}>
                  <span className="text-sm font-bold text-foreground/80">{seleccionado.estado === "abierto" ? "Cuadre pendiente" : `Diferencia · ${diferenciaTexto(seleccionado.diferenciaEfectivo || 0)}`}</span>
                  <span className="text-sm font-black text-foreground">{seleccionado.estado === "abierto" ? "—" : formatCurrency(seleccionado.diferenciaEfectivo || 0)}</span>
                </div>
              </div>
            </section>
            ...
            <section>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Ventas por medio de pago</h3>
              </div>
              {cargandoDetalle ? (
                <div className="rounded-xl bg-card/50 py-5 flex justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground/70" /></div>
              ) : ventasDetalle ? (
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={String(label)} className="rounded-xl bg-card/50 p-3">
                      <p className="text-[10px] text-muted-foreground/80 uppercase font-bold">{label}</p>
                      <p className="text-sm font-black text-foreground mt-1">{formatCurrency(Number(value))}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-muted-foreground">{errorDetalle || "No hay ventas calculadas para este turno."}</p>
              )}
            </section>
            ...
            <section>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Gastos y salidas reportadas</h3>
              </div>
              {egresosDetalle.length === 0 ? (
                <p className="rounded-xl bg-card/50 p-4 text-xs text-muted-foreground">No hay gastos de caja registrados para este turno.</p>
              ) : (
                <div className="rounded-xl bg-card/50 divide-y divide-border/50 overflow-hidden">
                  {egresosDetalle.map((egreso) => (
                    <div key={egreso.id} className="px-3 py-3 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{egreso.motivo || "Salida de caja"}</p>
                        <p className="text-[11px] text-muted-foreground/80 mt-1">{formatEgresoDate(egreso.fecha)} · Reportó: {egreso.cajeroNombre || "Sin identificar"}</p>
                      </div>
            ...
            {(seleccionado.notasApertura || seleccionado.notasCierre) && (
              <section>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-sm font-bold text-foreground">Notas del equipo</h3>
                </div>
                <div className="rounded-xl bg-card/50 p-3 space-y-3">
                  {seleccionado.notasApertura && <div><p className="text-[10px] text-muted-foreground/80 uppercase font-bold">Apertura</p><p className="text-sm text-foreground/70 mt-1">{seleccionado.notasApertura}</p></div>}
                  {seleccionado.notasCierre && <div><p className="text-[10px] text-muted-foreground/80 uppercase font-bold">Cierre</p><p className="text-sm text-foreground/70 mt-1">{seleccionado.notasCierre}</p></div>}
                </div>
              </section>
            )}
