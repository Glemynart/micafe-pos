...
export default function MermasPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Card className="bg-card/50 border-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground">Total perdido</p><p className="text-2xl font-bold text-destructive">{formatCurrency(total)}</p><p className="text-xs text-muted-foreground mt-1">{mermas.length} mermas</p></CardContent></Card>
      {mermas.length === 0 ? <Card className="bg-card/50 border-border"><CardContent className="py-8 text-center text-muted-foreground">Sin mermas</CardContent></Card> : (
        <div className="space-y-2">{mermas.map(m => (
          <Card key={m.id} className="bg-card/50 border-border"><CardContent className="p-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2"><Calendar className="h-3 w-3 text-muted-foreground" /><span className="text-xs text-muted-foreground">{ff(m.fecha)} · Reportó: {resolverNombreActor(m.registradoPor, m.registradoPorNombre, nombres)}</span></div>
