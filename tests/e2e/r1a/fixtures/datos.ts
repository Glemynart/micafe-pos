...
async function sembrarOperador(input: { empresaId: string; runId: string; sufijo: string; rol: "admin" | "cajero" | "supervisor"; permisos: string[] }): Promise<OperadorE2E> {
  ...
  const codigo = `r1a-${idSeguro(input.runId)}-${input.sufijo}`.toLowerCase();
...
export async function prepararFixtureR1A(nombre: string): Promise<FixtureR1A> {
  ...
  await db.collection("empresas").doc(empresaId).set({ estado: "activa", nombre: `Empresa ${runId}`, paisFiscal: "CO" });
  ...
  configuracion.caja = { ...configuracion.caja, rolesConTurnoObligatorio: ["cajero", "supervisor"], baseAperturaSugerida: 0 };
...
export async function limpiarFixtureR1A(fixture: Pick<FixtureR1A, "empresaId"> & Partial<Pick<FixtureR1A, "runId">>): Promise<void> {
  ...
  const colecciones = ["turnos", "turnos_activos", "operaciones_comandos", "operaciones_command_idempotency", "operaciones_auditoria", "credenciales_operativas", "membresias", "usuarios"];
