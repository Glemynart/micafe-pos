...
const safeHTML = (html) => DOMPurify.sanitize(html, { ALLOWED_TAGS: ['b','i','em','strong','br','span','div','tr','td','th','table','thead','tbody','button','h1','h2','h3','pre','p','u','small','input','select','option','label','img'], ALLOWED_ATTR: ['class','style','id','type','value','placeholder','src','alt','data-eliminar-producto','data-nombre','data-eliminar-proveedor'] });
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    document.body.innerHTML += safeHTML(`<div style="position:fixed;inset:0;background:red;color:white;padding:2rem;z-index:99999">
      <h1>Error Critico</h1>
      <pre>Error interno. Contacte al administrador.</pre>
    </div>`);
  }
});
...
function buildCats() {
  ...
  const bar = document.getElementById('cats-bar');
   bar.innerHTML = safeHTML(cats.map(c =>
...
function renderGrid() {
  ...
  grid.innerHTML = safeHTML(list.map(p => `
...
function showScanFlash(prod, errorCode) {
  ...
  if (prod) {
    ...
    flash.innerHTML = safeHTML(`
      ...
      <div class="sf-nombre">Codigo no encontrado</div>
...
function renderCarrito() {
  ...
  if (!carrito.length) {
   ...
   cont.innerHTML = safeHTML(carrito.map(item => `
    <div class="cart-item">
...
function mostrarTicket(data) {
  ...
  el('tk-num-fecha').innerHTML = safeHTML(`
  ...
  el('ticket-items').innerHTML = safeHTML(items.map(i => `
  ...
  // ── Totales ──────────────────────────────────────────
  el('ticket-totales').innerHTML = safeHTML(`
    <div class="t-totales">
...
async function imprimirTicket() {
  ...
  const html = `<html><head>
    ...
    <div class="nota-iva" style="margin-top:4px; font-size:8px;">MiTienda POS - Desarrollado por SaaS POS [REDACTED]</div>
...
function renderDashboard() {
  ...
  if (dashData.porMetodo && dashData.porMetodo.length) {
     metodosEl.innerHTML = safeHTML(dashData.porMetodo.map(m => `
  ...
  if (dashData.topProductos && dashData.topProductos.length) {
     topEl.innerHTML = safeHTML(dashData.topProductos.map((p, i) => `
      <div class="top-item">
  ...
  if (dashData.ultimasVentas && dashData.ultimasVentas.length) {
     lastEl.innerHTML = safeHTML(dashData.ultimasVentas.map(v => {
...
function renderInventario() {
  ...
  tbody.innerHTML = safeHTML(list.map(p => {
    ...
    return `
      <tr>
        ...
        <td>
          ...
          <button class="btn-table danger" data-eliminar-producto="${p.id}" data-nombre="${escAttr(p.nombre)}">🗑</button>
        </td>
      </tr>`;
  }).join('');
}
...
function renderHistorial(ventas) {
  ...
  const mIcon = { 'Efectivo': '💵', 'Transferencia': '📲', 'Débito': '💳', 'Crédito': '🏦' };
   cont.innerHTML = safeHTML(ventas.map(v => {
...
function mostrarTicketConCUFE(ticketData, feData) {
  ...
  document.getElementById('ticket-items').innerHTML = safeHTML(items.map(i =>
  ...
  document.getElementById('ticket-totales').innerHTML = safeHTML(`
    <div class="t-totales">
...
function renderProveedores() {
  ...
  if (!list.length) {
    tbody.innerHTML = safeHTML(`<tr><td colspan="7" style="text-align:center;color:var(--text2);padding:2rem">Sin resultados para "${q}"</td></tr>`);
    return;
  }
    ...
    return `
      <tr>
        ...
        <td>
          ...
          <button class="btn-table danger" data-eliminar-proveedor="${p.id}" data-nombre="${escAttr(p.nombre || '')}">🗑</button>
        </td>
      </tr>`;
  }).join('');
}
 ...
 *
 * Formatos soportados:
 *  1. Pipe-separated  → NIT: [REDACTED]|Nombre:Dist XYZ|Tel:3001234|Dir:Calle 1
...
function parsearCodigoFactura(raw) {
  ...
  // NIT: formatos 900.123.456-1 | 9001234561 | NIT: [REDACTED]
  if (!datos.nit) {
    const nitMatch =
...
function procesarTextoFactura(raw) {
  ...
  if (tieneDatos) {
    ...
    spGrid.innerHTML = safeHTML(campos.map(c => `
      <div class="sp-field">
...
// ── Event delegation para botones sanitizados ────────────
document.addEventListener('click', (e) => {
  const btn = e.target.closest('button');
  if (!btn) return;

  const id = btn.dataset.eliminarProducto;
  const nombre = btn.dataset.nombre || '';
  if (id && confirm(`Eliminar "${nombre}"?`)) eliminarProducto(parseInt(id), nombre);

  const pid = btn.dataset.eliminarProveedor;
  const pnombre = btn.dataset.nombre || '';
  if (pid && confirm(`Eliminar proveedor "${pnombre}"?`)) eliminarProveedor(parseInt(pid), pnombre);
});
