...
test("el build aislado excluye superficies con secretos o autoridad ajena", async () => {
  ...
  const required = [
    ...
    resolve(platformRoot, "operator-summary-callable.ts"),
  ...
  assert.equal(existsSync(resolve(sourceRoot, "lib", "functions", "src", "platform", "operator-summary-callable.js")), true);
});

test("el manifiesto aislado declara solo obtenerResumenOperadorSaas, sin params ni secretos", async () => {
  const manifest = await discover(sourceRoot);
  const endpoints = manifest.endpoints as Record<string, unknown>;
  assert.deepEqual(Object.keys(endpoints), ["obtenerResumenOperadorSaas"]);
...
test("saas-auth ya no declara obtenerResumenOperadorSaas", async () => {
  const manifest = await discover(resolve(workspaceRoot, "functions"));
  const endpoints = manifest.endpoints as Record<string, unknown>;
  assert.equal(Object.hasOwn(endpoints, "obtenerResumenOperadorSaas"), false);
});
