...
- **Estado:** ACTIVO.
 ...
 - **Siguiente PR recomendado:** preparar B3 de Eventos tenant-aware: transición controlada de documentos legacy, estrategia de compatibilidad y retiro posterior del modelo legacy, sin backfill ni escrituras productivas hasta una autorización específica. B2 no modificó `customDomain`, reservas, marketing ni landing fuera de la lectura pública. Las superficies Operator Portal/R1A/Electron/reservas-Wompi, impresión física, DIAN, Wompi, offline y notificaciones permanecen como seguimientos separados. `P0-07 / E3.1 — certificación física de impresión` permanece como gate externo bloqueado por hardware/canal.
- **Milestone activo:** `M4 — Certificación comercial`.
 - **Epic activo:** `E4.2 — Release readiness` permanece EN PROGRESO con PR #197 integrado; Storage, B1 y B2 de Eventos tenant-aware están certificados y el siguiente corte es B3, junto con seguimientos de dependencias y superficies externas. `E4.1 — Certificación integral` está COMPLETADO. `M3` conserva `E3.1 — Impresión física` como gate externo BLOQUEADO por hardware/canal y `E3.2` como condicional.
...
certificación tenant-aware. B1 y B2 de Eventos tenant-aware quedaron integrados bajo ADR-SAAS-025; el siguiente trabajo es preparar B3 como iniciativa separada, sin backfill ni escrituras productivas hasta autorización.
