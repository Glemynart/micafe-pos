...
- **Revision de ADR-SAAS-031:** PR #289 registro la revision de la politica de recovery y dejo la decision `NO ACEPTADO PARA EJECUCION`; no se habilitan PITR, schedules ni restores hasta definir y aceptar la politica completa.

- **Evidencia de release:** PR #291 preservo la recoleccion read-only del release observado en `origin/main @ 53ed743`; CI, Vercel y Functions quedaron PASS, mientras Rules/Storage independientes, smoke y recovery permanecen MISSING. Esta evidencia no autoriza escrituras ni inicia el Trial anual.
