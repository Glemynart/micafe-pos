...
class DB {
  ...
  async init() {
    ...
    if (userRes[0].values[0][0] === 0) {
      const tempPassword = process.env.MICAFE_TEMP_PASSWORD || 'CHANGE_ME';
  ...
  seedData() {
    ...
    const defaultConfig = [
      ...
      ['num_factura_fisica',   '1'],
      ['resolucion_vigencia',  ''],
      ['factus_base_url',      process.env.FACTUS_SANDBOX_BASE_URL      || ''],
      ['factus_client_id',     process.env.FACTUS_SANDBOX_CLIENT_ID     || ''],
      ['factus_client_secret', process.env.FACTUS_SANDBOX_CLIENT_SECRET || ''],
      ['factus_username',      process.env.FACTUS_SANDBOX_USERNAME      || ''],
      ['factus_password',      process.env.FACTUS_SANDBOX_PASSWORD      || ''],
      ['factus_rango_id',      ''],
    ];
