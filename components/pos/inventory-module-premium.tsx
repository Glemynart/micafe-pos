    ...
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card/80 backdrop-blur-xl p-6 rounded-[2rem] border border-border/50 shadow-sm">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-foreground flex items-center gap-3">
            Inventario 
            {espacioActivo && (
              <div className="flex items-center justify-center h-10 w-10 rounded-xl shadow-inner" style={{ backgroundColor: `${espacioActivo.color}20` }}>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Select value={categoriaFiltro} onValueChange={setCategoriaFiltro}>
            ...
            <SelectContent className="rounded-2xl border-border/50">
              <SelectItem value="todos" className="rounded-xl">Todas las categorías</SelectItem>
              {categorias.map(cat => (
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        {esCafeteria && (
          <div className="px-1 mt-2">
            <TabsList className="bg-secondary/40 p-1.5 rounded-2xl border border-border/30 inline-flex shadow-inner">
              <TabsTrigger value="products" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md gap-2 font-bold transition-all">
                <Package className="h-4 w-4" />
                Productos
              </TabsTrigger>
              <TabsTrigger value="ingredients" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md gap-2 font-bold transition-all">
        ...
        <TabsContent value="products" className="flex-1 mt-6 animate-fade-in">
          <Card className="h-full flex flex-col bg-card/60 backdrop-blur-xl border-border/50 rounded-[2rem] shadow-lg overflow-hidden">
            <div className="flex justify-between items-center bg-card/80 p-5 border-b border-border/50">
              ...
              <Button onClick={() => { setProductoAEditar(null); setShowProductDialog(true); }} className="h-11 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-5 shadow-lg shadow-primary/20 transition-all active:scale-95">
                        ...
                        >
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-2">
                              <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors" onClick={() => { setProductoAEditar(product); setShowProductDialog(true); }}>
        ...
        <TabsContent value="ingredients" className="flex-1 mt-6 animate-fade-in">
          <Card className="h-full flex flex-col bg-card/60 backdrop-blur-xl border-border/50 rounded-[2rem] shadow-lg overflow-hidden">
            <div className="flex justify-between items-center bg-card/80 p-5 border-b border-border/50">
              ...
              <Button onClick={() => { setInsumoAEditar(null); setShowIngredientDialog(true); }} className="h-11 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-5 shadow-lg shadow-primary/20 transition-all active:scale-95">
                <Plus className="h-5 w-5 mr-2" />
                Nuevo Insumo
              </Button>
            </div>
                ...
                <TableBody>
                  {filteredInsumos.map((insumo, idx) => {
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors" onClick={() => { setInsumoAEditar(insumo); setShowIngredientDialog(true); }}>
                            <Edit2 className="h-4 w-4" />
                          </Button>
                          <Button 
                            variant="ghost" 
      ...
      <Dialog open={showIngredientDialog} onOpenChange={setShowIngredientDialog}>
        <DialogContent className="bg-background border-border shadow-2xl max-w-lg p-0 gap-0 overflow-hidden sm:rounded-2xl">
          ...
          <div className="p-6 pt-4 border-t border-border/50 bg-muted/20">
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowIngredientDialog(false)} className="rounded-xl">
                Cancelar
              </Button>
              <Button onClick={handleCrearInsumo} className="bg-primary text-primary-foreground shadow-lg shadow-primary/20 rounded-xl active:scale-95 transition-all">
      ...
      <DialogContent className="bg-background border-border shadow-2xl max-w-lg p-0 gap-0 overflow-hidden sm:rounded-2xl">
        ...
        <div className="p-6 pt-4 border-t border-border/50 bg-muted/20">
          <DialogFooter>
            ...
            <Button className="bg-primary text-primary-foreground shadow-lg shadow-primary/20 rounded-xl active:scale-95 transition-all" onClick={handleCrearProducto}>
