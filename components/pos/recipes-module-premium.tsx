    ...
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card/80 backdrop-blur-xl p-6 rounded-[2rem] border border-border/50 shadow-sm">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Button onClick={handleOpenNewRecipe} className="h-12 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-6 shadow-lg shadow-primary/20 transition-all active:scale-95">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pr-4 mt-4 pb-10">
            {recetasMapeadas.map((recipe, idx) => {
              ...
              return (
                <Card 
                  key={recipe.id}
                  className="bg-card/60 backdrop-blur-sm border-border/50 shadow-sm hover:shadow-md hover:border-primary/30 transition-all duration-300 rounded-[2rem] overflow-hidden group animate-fade-in flex flex-col"
                  style={{ animationDelay: `${idx * 50}ms` }}
                >
                  <CardHeader className="pb-4 bg-gradient-to-b from-secondary/20 to-transparent">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-1 bg-background/80 backdrop-blur-sm p-1 rounded-xl shadow-sm border border-border/50">
                        <Button variant="ghost" size="icon" onClick={() => handleEditRecipe(recipe)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10">
                          <Edit2 className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteRecipe(recipe.id)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10">
                  ...
                  <CardContent className="space-y-5 flex-1 flex flex-col">
                    <div className="space-y-2.5 flex-1">
                      {recipe.ingredientes.slice(0, 3).map((ing, i) => (
                        <div key={i} className="flex items-center justify-between text-sm bg-secondary/20 px-3 py-2 rounded-xl">
                    ...
                    <Button 
                      variant="outline" 
                      className="w-full rounded-xl h-10 mt-auto border-border/50 hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors font-semibold"
                      onClick={() => {
      ...
      <Dialog open={showRecipeDialog} onOpenChange={setShowRecipeDialog}>
        <DialogContent className="bg-background border-border shadow-2xl max-w-2xl p-0 gap-0 overflow-hidden sm:rounded-2xl">
          ...
          <div className="space-y-5 px-6 py-5 overflow-y-auto max-h-[60vh] custom-scrollbar">
            ...
            {/* Summary */}
            <div className="flex items-center justify-between p-4 bg-primary/10 rounded-xl mt-4 border border-primary/20">
              <div>
          ...
          <div className="p-6 pt-4 border-t border-border/50 bg-muted/20">
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowRecipeDialog(false)} className="rounded-xl">
                Cancelar
              </Button>
              <Button onClick={handleSaveRecipe} className="bg-primary text-primary-foreground shadow-lg shadow-primary/20 rounded-xl active:scale-95 transition-all">
      ...
      <Dialog open={showProductionDialog} onOpenChange={setShowProductionDialog}>
        <DialogContent className="bg-background border-border shadow-2xl max-w-2xl p-0 gap-0 overflow-hidden sm:rounded-2xl">
          ...
          {selectedRecipe && (() => {
            ...
            return (
              <div className="px-6 py-5 overflow-y-auto max-h-[60vh] custom-scrollbar">
                ...
                <div className="mt-5 p-5 bg-primary/10 rounded-xl text-center border border-primary/20">
          ...
          <div className="p-6 pt-4 border-t border-border/50 bg-muted/20">
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowProductionDialog(false)} className="rounded-xl">
                Cerrar
              </Button>
            </DialogFooter>
          </div>
