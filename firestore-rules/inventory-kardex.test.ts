...
test("P1-01: ningún cliente puede crear una merma fuera de la callable", async () => {
  ...
  await expectDenied(admin.firestore().doc(merma).set({ empresaId: "empresa-a", cantidad: 1 }));
  await expectDenied(cajero.firestore().doc("mermas/p1-01-merma-cajero").set({ empresaId: "empresa-a", cantidad: 1 }));
});

test("P1-01: ningún cliente puede crear o modificar movimientos de inventario", async () => {
  const cajero = await contextFor(fixtures.tenantA.cajero);
  const marketing = await contextFor(fixtures.tenantA.marketing);
  const movimiento = "movimientos_inventario/p1-01-movimiento";

  await expectDenied(cajero.firestore().doc(movimiento).set({ empresaId: "empresa-a", cantidad: 1 }));
