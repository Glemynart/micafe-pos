...
const runId = process.env.E2E_B3_EVENTOS_RUN_ID ?? `b3-eventos-backfill-${Date.now()}`
const evidenceDir = resolve(process.env.E2E_B3_EVENTOS_EVIDENCE_DIR ?? `artifacts/e2e/b3-eventos-backfill/${runId}`)

if (!projectId.startsWith("demo-b3-eventos-backfill-")) throw new Error("B3-B E2E solo admite proyectos demo-b3-eventos-backfill-*.")
if (process.env.GOOGLE_APPLICATION_CREDENTIALS || process.env.FIREBASE_SERVICE_ACCOUNT) {
  throw new Error("B3-B E2E rechaza credenciales productivas.")
}
...
const result = spawnSync(process.execPath, [
  ...
  "--project",
  projectId,
  "node scripts/e2e/b3-eventos-backfill-inner.mjs",
