...
const projectId = process.env.E2E_P1_04_PROJECT_ID ?? "demo-p1-04-e2e";
const runId = process.env.E2E_P1_04_RUN_ID ?? `p1-04-${Date.now()}`;
const evidenceDir = resolve(process.env.E2E_P1_04_EVIDENCE_DIR ?? `artifacts/e2e/p1-04/${runId}`);
...
Object.assign(env, {
  ...
  E2E_P1_04_PROJECT_ID: projectId,
  E2E_P1_04_RUN_ID: runId,
  E2E_P1_04_EVIDENCE_DIR: evidenceDir,
...
const command = usarExistente
  ? [npm, ["--prefix", "functions", "run", "test:p1-04:emulator"]]
  : [process.execPath, [firebaseCli, "emulators:exec", "--only", "firestore", "--project", projectId, "npm --prefix functions run test:p1-04:emulator"]];
