...
const runId = process.env.E2E_B3_CLOSURE_RUN_ID ?? `b3-eventos-closure-${Date.now()}`
const evidenceDir = resolve(process.env.E2E_B3_CLOSURE_EVIDENCE_DIR ?? `artifacts/e2e/b3-eventos-closure/${runId}`)

if (!projectId.startsWith("demo-b3-eventos-closure-")) throw new Error("B3 closure E2E solo admite proyectos demo-b3-eventos-closure-*.")
if (process.env.GOOGLE_APPLICATION_CREDENTIALS || process.env.FIREBASE_SERVICE_ACCOUNT) {
  throw new Error("B3 closure E2E rechaza credenciales productivas.")
}
...
const reservarPuerto = () => new Promise((resolvePort, reject) => {
  ...
  server.listen(0, "127.0.0.1", () => {
    ...
    if (!address || typeof address === "string") {
      server.close(() => reject(new Error("No se pudo reservar un puerto de Emulator.")))
      return
    }
...
Object.assign(env, {
  ...
  E2E_B3_CLOSURE_PROJECT_ID: projectId,
  E2E_B3_CLOSURE_RUN_ID: runId,
  E2E_B3_CLOSURE_EVIDENCE_DIR: evidenceDir,
...
const result = spawnSync(process.execPath, [
  ...
  "--project",
  projectId,
  "node --import tsx scripts/e2e/b3-eventos-closure-inner.ts",
