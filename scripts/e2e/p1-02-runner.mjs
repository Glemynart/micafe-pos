...
const projectId = process.env.E2E_P1_02_PROJECT_ID ?? "demo-p1-02-e2e";
const runId = process.env.E2E_P1_02_RUN_ID ?? `p1-02-${Date.now()}`;
const evidenceDir = resolve(process.env.E2E_P1_02_EVIDENCE_DIR ?? `artifacts/e2e/p1-02/${runId}`);
...
Object.assign(env, {
  ...
  E2E_P1_02_PROJECT_ID: projectId,
  E2E_P1_02_RUN_ID: runId,
  E2E_P1_02_EVIDENCE_DIR: evidenceDir,
...
const command = usarExistente
  ? [npm, ["--prefix", "functions", "run", "test:p1-02:emulator"]]
  : [process.execPath, [firebaseCli, "emulators:exec", "--only", "firestore", "--project", projectId, "npm --prefix functions run test:p1-02:emulator"]];
